package com.keeptoken.p2p.peer.network;

import com.keeptoken.p2p.peer.Config;
import com.keeptoken.p2p.peer.Peer;
import com.keeptoken.p2p.peer.network.message.Handshake;
...
public class PeerChannelHandler extends SimpleChannelInboundHandler<Message> {
    ...
    public void channelRead0(final ChannelHandlerContext ctx, final Message message) throws Exception {
        LOGGER.debug("AgentMessage {} received from {}", message.getClass(), ctx.channel().remoteAddress());
        final Connection connection = getSessionAttribute(ctx).get();
